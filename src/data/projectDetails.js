export const projectDetails = {
  ...
  portfolio: {
    ...
    problem: {
      ...
      en: 'The design didn\'t reflect my aesthetic sensibility. There were no availability signals or direct CV access — two things recruiters look for in the first few seconds. And navigation between sections had friction: there was no way back from Contact, and the "← Work" button typography was inconsistent.',
    },
    objectives: {
      es: [
        'Crear una primera impresión visual que no se parezca a ningún otro portfolio UX/UI',
        'Comunicar disponibilidad, contacto y CV en menos de 5 segundos',
        'Resolver todos los problemas de navegación y coherencia tipográfica',
        'Diseñar una página de CV integrada, imprimible y bilingüe',
        'Que cada decisión de diseño tenga un porqué claro, no sea decorativa',
      ],
      en: [
        'Create a visual first impression unlike any other UX/UI portfolio',
        'Communicate availability, contact and CV access in under 5 seconds',
        'Fix all navigation and typographic consistency issues',
        'Design an integrated, printable, bilingual CV page',
        'Every design decision must have a clear reason, not just be decorative',
      ],
    },
    roleText: {
      es: 'Todas las decisiones de diseño son mías: referencias, identidad visual, tipografía, sistema de color, layout, interacciones y contenido. La implementación del código la hice en colaboración con IA (Claude) — yo definía qué construir y cómo debía comportarse, la IA lo ejecutaba. Cada resultado lo revisaba, corregía y validaba yo. Este flujo me permitió iterar muy rápido sin perder el control del criterio.',
      en: 'All design decisions are mine: references, visual identity, typography, colour system, layout, interactions and content. I handled the code implementation in collaboration with AI (Claude) — I defined what to build and how it should behave, the AI executed it. I reviewed, corrected and validated every result. This workflow allowed me to iterate very fast without losing design control.',
    },
    process: {
      es: [
        ...
        'Tipografía: Geist (sans-serif técnica, sin decoración innecesaria)',
        'Página Work: lista grande con reveal de thumbnail en hover — el contenido manda',
        'Corrección de navegación: logo visible en Contacto, botón ← Trabajo siempre presente',
        'Página CV: imprimible con Cmd+P, bilingüe ES/EN, coherente con el estilo del portfolio',
      ...
      en: [
        ...
        'Typography: Geist (technical sans-serif, no unnecessary decoration)',
        'Work page: large list with thumbnail reveal on hover — content leads',
        'Navigation fix: visible logo in Contact, ← Work button always present',
        'CV page: printable with Cmd+P, bilingual ES/EN, consistent with portfolio style',
        'Testing session with real recruiter feedback in real time',
      ],
    },
    result: {
      es: 'Un portfolio que no parece un portfolio estándar. La primera impresión es visual e inmediata: vídeo, tipografía de gran formato, información clave en los cuatro rincones. Ningún elemento es decorativo: el vídeo establece el tono, las esquinas organizan la información por jerarquía, el verde señaliza disponibilidad sin gritar. El recruiter encuentra el CV, el email y LinkedIn sin tener que buscar.',
      en: 'A portfolio that doesn\'t look like a standard portfolio. The first impression is visual and immediate: video, large-format type, key information in all four corners. No element is decorative: the video sets the tone, the corners organise information by hierarchy, the green signals availability without shouting. The recruiter finds the CV, email and LinkedIn without having to look for them.',
    },
    reflection: {
      es: '"Diseñar tu propio portfolio es el proyecto más difícil porque el cliente eres tú y los requisitos no tienen límite. Lo más valioso no fue el resultado final, sino entender en tiempo real qué busca un recruiter y ajustar las decisiones sobre la marcha."',
      en: '"Designing your own portfolio is the hardest project because the client is yourself and the requirements have no limit. The most valuable part wasn\'t the final result — it was understanding in real time what a recruiter looks for and adjusting decisions on the fly."',
    },
    prototype: null,
    ...
    gallery: [
      ...
      },
      {
        ...
        desc: { es: 'Vídeo de naturaleza como fondo: establece un tono diferente al 100% de portfolios de diseño. Tipografía anclada en esquinas: rompe la cuadrícula centrada y organiza la info por zonas. Verde pulsante solo para disponibilidad: señal inmediata sin ruido.', en: 'Nature video as background: sets a tone different from 100% of design portfolios. Corner-anchored type: breaks the centred grid and organises info by zones. Pulsing green only for availability: immediate signal without noise.' },
        image: './projects/portfolio-hero.png',
      },
      {
        title: { es: 'Página de trabajo — lista editorial', en: 'Work page — editorial list' },
        desc: { es: 'Tipografía grande como elemento visual principal. Los thumbnails están ocultos y aparecen en hover para no saturar: el contenido importa más que la imagen. El hover negro con texto en blanco refuerza el sistema de color.', en: 'Large typography as the main visual element. Thumbnails are hidden and appear on hover to avoid visual noise: content matters more than the image. The black hover with white text reinforces the colour system.' },
        image: './projects/portfolio-work.png',
      },
      {
        title: { es: 'Detalle de proyecto — estructura narrativa', en: 'Project detail — narrative structure' },
        desc: { es: 'Cada proyecto sigue la misma estructura: contexto → problema → objetivos → rol → proceso → galería → resultado → reflexión. No es decorativa, es UX: el recruiter sabe siempre dónde está y qué viene después.', en: 'Every project follows the same structure: context → problem → objectives → role → process → gallery → result → reflection. Not decorative — it\'s UX: the recruiter always knows where they are and what comes next.' },
